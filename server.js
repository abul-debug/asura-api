...
// ==========================================
// ADVANCED HYBRID ENGINE V2 (JAVASCRIPT)
// ==========================================
...
class AdvancedHybridEngineV2 {
  ...
  predict(input) {
    ...
    if (values.length < this.minHistory) {
      return { prediction: 'WAIT', confidence: 0, probabilityBig: 0.5, edge: 0, regime: 'BALANCED', decision: 'WAIT' };
    }
    ...
    const prediction = edge >= this.minEdge && agreement >= 0.58 && regime !== 'SHIFTING' ? (probabilityBig >= 0.5 ? 'BIG' : 'SMALL') : 'WAIT';
    ...
    return {
      prediction,
      decision: prediction === 'BIG' ? 'B' : (prediction === 'SMALL' ? 'S' : 'WAIT'),
...
app.get('/', (req, res) => res.send("Advanced Hybrid Engine V2 API Live!"));
